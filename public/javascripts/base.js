function ishare(api, pubid)
{
	...
	switch(api)
	{
		case 'qr':
			...
			$('#QRShareImage').attr("src","//api.qrserver.com/v1/create-qr-code/?size=200x200&data="+encodeURIComponent(window.location.href));
			break;
		...
		default:
			window.open('//api.addthis.com/oexchange/0.8/forward/'+ api +'/offer?pubid='+pubid+'&url='+encodeURIComponent(window.location.href)+'&title='+encodeURIComponent(document.title));
	}
}
		...
		$(function () {
			$(window).scroll(function () {
				if ($(this).scrollTop() > show()) {
					$("#prev, #next, #download, #play").fadeIn();
				} else {
					$("#prev, #next, #download, #play").fadeOut();
				}
			});
		});
	});
